...
void handleClient() {
  if (wifiEnabled) {
    ...
    if (client) {
      ...
      if (decryptedMessage.startsWith("Alerte!")) {
        handleAlert(decryptedMessage, clientIP);       // Gérer un message d'alerte
      } else {
...
String messageDecryption(String messageToDecrypt) {
  char *upedMessage = new char[messageToDecrypt.length() + 1];  // Réservation d'une mémoire de taille `messageToDecrypt.length() + 1` pour le tableau `upedMessage`.
  messageToDecrypt.toCharArray(upedMessage, messageToDecrypt.length() + 1); // La valeur `+1` permet de réserver de la place pour le caractère nul (`'\0'`), qui marque la fin de la chaîne.
  ...
  for (int i = 0; i < lengthMessage; i++) {
    if (upedMessage[i] == '\0') {
      ...
      decryptedMessage[i] = (char) (asc - 26 * ((asc < 65) - (asc > 90)));
    }
  }
