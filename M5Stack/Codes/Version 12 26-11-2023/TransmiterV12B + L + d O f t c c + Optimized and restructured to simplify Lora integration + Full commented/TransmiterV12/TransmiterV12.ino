...
void loop() {
  ...
  data.humidity = readHumidity();  // Lecture de la température
  ...
  if (humindex > maxHumindex) {
    ...
    messageToSend = createRegularMessage(data.temperature, data.humidity, humindex);  // Création du message régulier
  }
  ...
  delay(5000);                       // Attente ajusté de 5 secondes avant la prochaine itération/envoi de message pour donner le temps à la personne d'appuyer sur le boutton
}
...
float readTemperature() {
  sht.get();                          // Obtenir les données du capteur de température
...
float readHumidity() {
  sht.get();                          // Obtenir les données du capteur d'humidité
  return sht.humidity;
}
...
String createRegularMessage(float temperature,float humidity, float humindex) {
  return signature + "SSID :" + String(roomName) + " Température : " + String(temperature) + "°C, Humidity : " + String(humidity) + "% & Humidex : " + String(humindex) + " Message " + String(messageCounter++);
}
